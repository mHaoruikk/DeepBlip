...
from src.models.g_transformer import G_transformer
...
def main(args: DictConfig):
    ...
    run_names = [f'g-trfm-conf={conf_strength}_m={n_periods}', f'g-trfm-base-conf={conf_strength}_m={n_periods}']
    capo_pred_dict = dict()
    for i in range(2):
        ...
        if i == 0:
            logger.info("Build G-Computation model G-transformer with intervention")
        else:
            logger.info("Build G-Computation model G-transformer with baseline (null treatment)")
        ...
        if args.exp.load_pretrained:
            ...
            g_trfm = G_transformer.load_from_checkpoint(checkpoint_path=checkpoint_path)
        ...
        if args.checkpoint.save:
            checkpoint_callback = ModelCheckpoint(dirpath = artifacts_path,           
                filename = "g-trfm-{epoch}-{val_loss:.4f}",
        ...
        if (args.exp.resume_train) or (args.exp.load_pretrained == False):
            trainer.fit(g_trfm, train_loader, val_loader)

        testloader = DataLoader(data_pipeline.test_data, batch_size=args.exp.batch_size, shuffle=False)
        capo_pred, active_mask = g_trfm.predict_capo(testloader)
