...
from src.models.g_net import GNet
...
def main(args: DictConfig):
    ...
    if args.exp.logging:
        ...
        mlf_logger = FilteringMlFlowLogger(filter_submodels=[], experiment_name=experiment_name,
            tracking_uri=args.exp.mlflow_uri, run_name=f"gnet_conf={conf_strength}_m={n_periods}")
    ...
    if args.exp.load_pretrained:
        ...
        gnet = GNet.load_from_checkpoint(checkpoint_path=checkpoint_path)
    ...
    if args.checkpoint.save:
        checkpoint_callback = ModelCheckpoint(dirpath = artifacts_path,           
            filename = "gnet-{epoch}-{val_loss:.4f}",
    ...
    capo_intv = gnet.predict_capo(testloader, T_intv_disc, T_intv_cont)
    capo_base = gnet.predict_capo(testloader, T_base_disc, T_base_cont)
